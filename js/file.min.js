...
this.options.map.googlemap.fitBounds(bounds);}}},this));};File.prototype.updatePid=function(){if(this.options.map.options.pid!==''){var results=[];$.post(this.options.map.options.ajax,{action:'ibs_mappro_pid',pid:this.options.map.options.pid,},$.proxy(function(data,status){if(status==='success'){for(var id in data){for(var pid in this.placemarks){if(this.placemarks[pid].options.name===id){results.push([pid,data[id]]);}}}
