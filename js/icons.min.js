...
return result;};Map.prototype.getIconOptions=function(list){$.get(this.options.ajax,{'action':'ibs_mappro_icon_palette'},function(data,status){$(list).empty();for(var i in data){var bg="background:url(xxx) left top no-repeat; vertical-align:middle;".replace(/xxx/,data[i].url);$(list).append($('<li class="placemark-select-item" >').attr('style',bg).append($('<input>').addClass('icon-option-name').attr({disabled:true,'type':'text','value':data[i].name})).append($('<input>').addClass('icon-option-value').attr({'type':'hidden','value':data[i].url})))}},'json');};})(jQuery);