...
var ext=getExtension(fileName).toLowerCase();if(ext==='kml'||ext==='kmz'||ext==='gpx'){var dir=map.html['upload-dialog'].find('.upload-target-directory').val();dir=map.siteUrl()+'maps/'+dir;map.file.importFile(dir+fileName);}});this.html['data'].on('click','.chart-close',{map:this},function(event){event.data.map.elevationSegment=null;event.data.map.sizeHtml();})
this.html['file-dialog'].find('.reset-lines').button({text:true,label:'Set Lines',icons:{primary:'ui-icon-arrowreturnthick-1-e'}});this.html['upload-dialog'].on('change','.upload-target-directory',{map:this},function(event){var map=event.data.map;var newdir=map.html['upload-dialog'].find('.upload-target-directory').val();newdir=newdir+"/";newdir=newdir.replace('//','/');map.html['upload-dialog'].find('.upload-target-directory').val(newdir.replace(map.siteRoot()+'maps/',''));map.uploader.fineUploader('setParams',{'dir':newdir});});var init_height=this.options.height==='100%'?$(window).height():this.options.height;var init_width=this.options.width==='100%'?$(window).width():this.options.width;this.html['ibs-container'].width(init_width);this.html['ibs-container'].height(init_height);this.html['ibs-container'].on('resizestop','',{map:this},function(event){event.data.map.sizeHtml();});this.html['list'].on('resizestop','',{map:this},function(event){event.data.map.sizeHtml();});this.html['ibs-header'].on('click','.ibs-size',{map:this},function(event){event.data.map.resetSize();});this.resetSize();};Map.prototype.resetSize=function(){var init_height=this.options.height==='100%'?$(window).height():this.options.height;var init_width=this.options.width==='100%'?$(window).width():this.options.width;this.html['ibs-container'].css({'top':this.options.top,'left':this.options.left})
