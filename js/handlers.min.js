...
obj.map.elevation(obj.segment);});this.html['dd-options'].on('click','.control-followroad',{map:this},function(event){var onoff=event.data.map.html['dd-options'].find('.control-followroad').is(':checked');event.data.map.options.followroad=onoff;});this.html['dd-options'].on('click','.control-avoidhighways',{map:this},function(event){event.data.map.options.avoidhighways=event.data.map.html['dd-options'].find('.control-avoidhighways').is(':checked');});this.html['dd-options'].on('change','input[name=travelmode]',{map:this},function(event){event.data.map.options.travelmode=event.data.map.html['dd-options'].find('input[name=travelmode]:checked').val();});this.html['placemark-dialog'].on('click','.placemark-select-item',{map:this},function(event){event.data.map.html['placemark-dialog'].find('.placemark-selected-icon').attr('src',$(this).find('.icon-option-value').val());});}
