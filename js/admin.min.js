...
var parent=$(this).parents('li')[0];$(parent).find('.current-icon-url').val(url);$(parent).attr('style',bg);}});}});$('#current-icon-list').on('click','.icon-delete',{},function(event){$(this).parents('li').eq(0).remove();reorderIcons();});$('#ibs-icon-upload').append($('<div id="upload-dialog" >').addClass('hide dialog-map').attr('title','Upload Icons').append($('<div id="upload-browser" >')).append($('<div>').append($('<span>').append($('<div>').addClass('section-header').html('Target')).append($('<div>').addClass('kmlkmzgpx').append($('<input id="upload-target-directory" >').attr({type:"text",size:"75",value:"/"}))))).append($('<div>').append($('<span>').append($('<div>').addClass('section-header').css({'font-size':'.9em','padding-left':'5px'}).html('status')).append($('<div >').addClass('map-upload-div file-list-div').css('height','100px').append($('<ul id="upload-list" >').addClass(' file-list')))).append($('<div>').css('display','none').append($('<div id="upload-button" >').html('Browse')))));var uploader=$('#upload-button').fineUploader({listElement:$('#upload-list'),request:{endpoint:ibs_mappro.site+'js/jquery.fineuploader/server/endpoint.php',params:{'dir':ibs_mappro.root+'maps/upload/'}},debug:false});uploader.on('complete','',{},function(event,id,fileName,responseJSON){var map=event.data.map;var lib=$('#upload-target-directory').val().replace('/','');var has=false;$('.icon-lib').each(function(index,item){if($(item).val()===lib){has=true;}});if(false===has){$('#icon-library-select').prepend($('<option>').attr({'value':lib}).addClass('icon-lib').text(lib).trigger('change'));}
$('#icon-library-select').val(lib);$('#icon-library-select').trigger('change');});$('#upload-target-directory').on('change','',{},function(event){var root=ibs_mappro.root+'icons/';var newdir=$('#upload-target-directory').val().replace(root,'');if(newdir.indexOf(root)===-1){newdir=root+newdir;}
if(newdir===root){newdir=root+'new/';}
newdir=newdir+"/";newdir=newdir.replace('//','/');var dir_file=newdir.replace(root,'');$('#upload-target-directory').val(dir_file).focus();uploader.fineUploader('setParams',{'dir':newdir});});$('#ibs-upload-action').on('click','',{},function(event){var dialog=$('#upload-dialog');dialog.dialog({position:{my:"center",at:"top",of:window},autoOpen:true,width:'auto',modal:true,buttons:{'Select Files':$.proxy(function(){var a=$('#upload-button');var b=a.find('input[qq-button-id]');var c=a.find('input');c.trigger('click');},this),Clear:function(){$('#upload-list').empty();},Close:function(){$(this).dialog('close');}},close:function(){$(this).dialog('destroy');},open:function(){var initdir=ibs_mappro.root+'icons/new/';$('#upload-target-directory').val('new/');uploader.fineUploader('setParams',{'dir':initdir});$('#upload-browser').fileTree({root:ibs_mappro.root+'icons/',script:ibs_mappro.ajax+'?action=ibs_mappro_folders&type=dir',folderEvent:'click',expandSpeed:1000,collapseSpeed:1000,multiFolder:false},function(file){alert(file);},function(dir){$('#upload-target-directory').val(dir);$('#upload-target-directory').trigger('change');});}});});$('#ibs-icon-clean').append($('<div id="clean-dialog" >').addClass('hide dialog-map').attr('title','Manage Icon Folders').append($('<div id="clean-browser" >')).append($('<div>').append($('<span>').append($('<div>').addClass('section-header').html('Target')).append($('<div>').addClass('kmlkmzgpx').append($('<input id="clean-target-directory" >').attr({type:"text",size:"75",value:"/",disabled:true}))))));$('#ibs-clean-action').on('click','',{},function(event){$('#clean-target-directory').val('')
$('#clean-dialog').dialog({position:{my:"center",at:"center",of:window},autoOpen:true,width:'auto',modal:true,buttons:{'Remove Files':function(){var files='';$('#clean-browser').find('LI A.selected').each(function(index){files+=$(this).attr('rel')+';';});if(files.length>0){$.get(ibs_mappro.ajax,{'action':'ibs_mappro_remove','type':'files','dir':ibs_mappro.root+'icons/'+$('#clean-target-directory').val(),'files':files},function(data,status){$('#clean-browser').find('LI A.selected').remove();},'TEXT');}},'Remove Folder':function(){$.get(ibs_mappro.ajax,{action:'ibs_mappro_remove',type:'directory','dir':ibs_mappro.root+'icons/'+$('#clean-target-directory').val()},function(data,status){$('#clean-browser').find('LI.expanded').remove();var lib=$('#clean-target-directory').val().replace('/','');$('.icon-lib').each(function(index,item){if($(item).val()===lib){$(item).remove();}});},'TEXT');},Close:function(){$(this).dialog('close');}},close:function(){$(this).dialog('destroy');},open:function(){$('#clean-browser').fileTree({root:ibs_mappro.root+'icons/',script:ibs_mappro.ajax+'?action=ibs_mappro_folders&type=all',folderEvent:'click',expandSpeed:0,collapseSpeed:0,multiFolder:false},function(file){if(event.shiftKey){var first=false;var found=false;var rel=$(this).attr('rel');$(this).parent().parent().find('A').each(function(n){if(!found||!first){if($(this).attr('rel')===rel){found=true;}else{if($(this).hasClass('selected')&&!first){first=true;}}
if(first||found){$(this).addClass('selected');}}else{$(this).removeClass('selected');}});}else{if(event.ctrlKey){$(this).toggleClass('selected');}else{var has=$(this).hasClass('selected');$(this).parent().parent().find('A').removeClass('selected');if(false===has){$(this).addClass('selected');}}}},function(dir){$('#clean-target-directory').val(dir.replace(ibs_mappro.root+'icons/',''));});}});});$('#ibs-map-clean').append($('<div id="map-dialog" >').addClass('hide dialog-map').attr('title','Manage Map Folders').append($('<div id="map-browser" >')).append($('<div>').append($('<span>').append($('<div>').addClass('section-header').html('Target')).append($('<div>').addClass('kmlkmzgpx').append($('<input id="map-target-directory" >').attr({type:"text",size:"75",value:"/",disabled:true}))))));$('#ibs-map-manage').on('click','',{},function(event){$('#map-target-directory').val('')
$('#map-dialog').dialog({position:{my:"center",at:"center",of:window},autoOpen:true,width:'auto',modal:true,buttons:{'Remove Files':function(){var files='';$('#map-browser').find('LI A.selected').each(function(index){files+=$(this).attr('rel')+';';});if(files.length>0){$.get(ibs_mappro.ajax,{'action':'ibs_mappro_remove','type':'files','dir':ibs_mappro.root+'maps/'+$('#map-target-directory').val(),'files':files},function(data,status){$('#map-browser').find('LI A.selected').remove();},'TEXT');}},'Remove Folder':function(){$.get(ibs_mappro.ajax,{action:'ibs_mappro_remove',type:'directory','dir':ibs_mappro.root+'maps/'+$('#map-target-directory').val()},function(data,status){$('#map-browser').find('LI.expanded').remove();var lib=$('#map-target-directory').val().replace('/','');},'TEXT');},Close:function(){$(this).dialog('close');}},close:function(){$(this).dialog('destroy');},open:function(){$('#map-browser').fileTree({root:ibs_mappro.root+'maps/',script:ibs_mappro.ajax+'?action=ibs_mappro_folders&type=all',folderEvent:'click',expandSpeed:0,collapseSpeed:0,multiFolder:false},function(file){if(event.shiftKey){var first=false;var found=false;var rel=$(this).attr('rel');$(this).parent().parent().find('A').each(function(n){if(!found||!first){if($(this).attr('rel')===rel){found=true;}else{if($(this).hasClass('selected')&&!first){first=true;}}
if(first||found){$(this).addClass('selected');}}else{$(this).removeClass('selected');}});}else{if(event.ctrlKey){$(this).toggleClass('selected');}else{var has=$(this).hasClass('selected');$(this).parent().parent().find('A').removeClass('selected');if(false===has){$(this).addClass('selected');}}}},function(dir){$('#map-target-directory').val(dir.replace(ibs_mappro.root+'maps/',''));});}});});$('#shortcode-browser').fileTree({root:ibs_mappro.root+'maps/',script:ibs_mappro.ajax+'?action=ibs_mappro_folders&type=map',folderEvent:'click',expandSpeed:0,collapseSpeed:0,multiFolder:false},function(file){file=file.replace(ibs_mappro.root,ibs_mappro.site);$('#shortcode-url').val(file);$('#shortcode-url').trigger('change');},function(dir){});$('#shortcode-options').on('change','.shortcode-input',{},function(event){var result='[ibs-mappro ';$('#shortcode-options').find('select').each(function(index,item){if($(item).val()!==''){result+=$(item).attr('name')+'="'+$(item).val()+'" ';}});$('#shortcode-options').find('input').each(function(index,item){if($(item).val()!==''){result+=$(item).attr('name')+'="'+$(item).val()+'" ';}});$('#shortcode').val(result+']');});});