...
(function($){Map.prototype.init=function(arg){this.options={address:'',ajax:null,site:null,root:null,div:'body',top:0,left:0,height:'100%',width:'100%',url:'',mid:'M1001',metric:false,filename:'',location:new google.maps.LatLng(37.09024,-95.712891),strokeColor:'#000fff',strokeWeight:5,strokeOpacity:0.3,distance:false,followroad:true,travelmode:"DRIVING",avoidhighways:true,user_type:'guest',user_name:'unknown',pid:'',gpx_routes:false,gpx_tracks:true,gpx_waypoints:true,gpx_placemarks:true,mode:'edit'};for(var attr in arg){this.options[attr]=arg[attr];}
this.siteRoot=function(){return this.options.root;};this.siteUrl=function(){return this.options.site;};this.html=[];this.garmin_symbols=null;this.files={};this.fileId=1000;this.googlemap=null;if(typeof window.maps==='undefined'){window.maps={};}
...
Map.prototype.alert=function(message){var dd=jQuery('<div>').attr({'id':'alert-dialog','title':'Alert'}).css({'background-color':'Tomato','color':'white'}).append(jQuery('<p>').append(jQuery('<img>').attr('src',this.siteUrl()+'image/alert-white.png').css({'float':'left','margin-right':'5px'})).append(jQuery('<span>').html(message)));jQuery(dd).dialog({position:['center','top'],autoOpen:true,modal:true,buttons:{Ok:function(){jQuery(this).dialog("close");}},close:function(){jQuery(this).dialog('destroy');}});};Map.prototype.confirm=function(message,callback){var dd=jQuery('<div>').attr({'id':'alert-dialog','title':'Confirm'}).css({'background-color':'Tomato','color':'white'}).append(jQuery('<p>').append(jQuery('<img>').attr('src',this.siteUrl()+'image/alert-white.png').css({'float':'left','margin-right':'5px'})).append(jQuery('<span>').html(message)));jQuery(dd).dialog({position:['center','top'],autoOpen:true,modal:true,buttons:{Ok:function(){jQuery(this).dialog("close");callback(true);},Cancel:function(){jQuery(this).dialog("close");callback(false);}},close:function(){jQuery(this).dialog('destroy');}});}
Map.prototype.prompt=function(message,callback){var dd=jQuery('<div>').attr({'id':'alert-dialog','title':'Alert'}).css({'background-color':'AliceBlue','color':'blue'}).append(jQuery('<p>').append(jQuery('<img>').attr('src',this.siteUrl()+'image/question.png').css({'float':'left','margin-right':'5px'})).append(jQuery('<span>').css({'float':'left','margin-right':'5px'}).html(message)).append(jQuery('<input>').attr({'id':'answer','type':'text','size':'30','value':''})));jQuery(dd).dialog({position:['center','top'],autoOpen:true,modal:true,buttons:{Ok:function(){jQuery(this).dialog("close");if(typeof callback==='function'){callback(jQuery(dd).find('#answer').val());}},Cancel:function(){jQuery(this).dialog("close");if(typeof callback==='function'){callback(null);}},},close:function(){jQuery(this).dialog('destroy');}});}
Map.prototype.notice=function(message){var dd=jQuery('<div>').attr({'id':'notice-dialog','title':'Notice'}).css({'background-color':'AliceBlue','color':'blue'}).append(jQuery('<p>').append(jQuery('<img>').attr('src',this.siteUrl()+'image/notice.png').css({'float':'left','margin-right':'5px'})).append(jQuery('<span>').html(message)));jQuery(dd).dialog({position:['center','top'],autoOpen:true,modal:false,show:{effect:"blind",duration:1000},hide:{effect:"blind",duration:1000},open:function(){var noticeInterval=setInterval(function(){jQuery(dd).dialog('close');clearInterval(noticeInterval);},2500);},close:function(){jQuery(this).dialog('destroy');}});}})(jQuery);